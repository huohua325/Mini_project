...
public class ShapePanel extends BaseTaskPanel implements TaskPanelInterface {
    ...
    private void showCurrentShape() {
        if ((is2DMode && currentShapeIndex < SHAPES_PER_TEST) || 
            (!is2DMode && currentShapeIndex < SHAPES_PER_TEST)) {
    ...
    public void handleSubmit() {
        ...
        if (correct) {
            if (is2DMode) shapeRecognition.incrementCorrectCount2D();
            else shapeRecognition.incrementCorrectCount3D();
        }
    ...
    protected int calculateScore() {
        ...
        for (int i = 0; i < correctAnswers.size(); i++) {
            if (correctAnswers.get(i)) {
                ...
                if (attempts <= 3) {
                    if (is2DMode) {
                        totalScore += attempts == 1 ? 3 : attempts == 2 ? 2 : 1;
                        totalPossibleScore += 3;
                    } else {
                        totalScore += attempts == 1 ? 5 : attempts == 2 ? 3 : 1;
        ...
        return totalPossibleScore == 0 ? 0 : (int)((double)totalScore / totalPossibleScore * 100);
    }
    ...
    public String getFeedback() {
        ...
        feedback.append(is2DMode ? "2D形状识别测试统计：\n" : "3D形状识别测试统计：\n");
        ...
        for (int i = 0; i < attemptsPerTask.size(); i++) {
            if (correctAnswers.get(i) && attemptsPerTask.get(i) <= 3) {
                correctWithinThree++;
                if (attemptsPerTask.get(i) == 1) perfectCount++;
            }
        }
        ...
        feedback.append("\n").append(score >= 90 ? "太棒了！你对这些形状已经非常熟悉了！" :
                                   score >= 70 ? "不错的表现！继续练习可以做得更好！" :
                                               "继续加油！多加练习一定能提高！");
